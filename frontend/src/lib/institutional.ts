...
const CONTENT: Record<Locale, InstitutionalContent> = {
  ...
  pt: {
    ...
    steps: [
      ...
      { title: "Autorize o que importa", body: "Quando um Action precisa de autoridade humana, a execução pausa e pergunta antes da ação consequente." },
    ],
  ...
  zh: {
    ...
    modelKicker: "Shimpz // 一个有用的心智模型", modelTitle: "一次对话。由你选择的能力。", modelLead: "只有在帮助你决定下一步时，产品模型才会出现。", steps: [{ title: "与 Team 对话", body: "Team 隔离保存对话、上下文、文件、推理配置和工作负载边界。" }, { title: "安装 Assistant", body: "每个独立发布的 Assistant 都为 Team 提供已声明且可审查的能力。" }, { title: "授权重要操作", body: "当 Action 需要人类授权时，执行会暂停，并在关键操作前询问你。" }],
  ...
  ja: {
    ...
    modelKicker: "Shimpz // 役立つメンタルモデル", modelTitle: "一つの対話。選べる能力。", modelLead: "製品モデルは、次の判断に役立つときだけ前面に現れます。", steps: [{ title: "Team と話す", body: "Team は対話、コンテキスト、ファイル、推論設定、ワークロード境界を分離します。" }, { title: "Assistant を導入する", body: "独立して公開された各 Assistant が、宣言済みで確認可能な能力を Team に与えます。" }, { title: "重要な部分を承認する", body: "Action に人の権限が必要な場合、実行は重要な操作の前で止まり、確認を求めます。" }],
...
const PAGE_CONTENT: Record<InstitutionalPage, Record<Locale, InstitutionalPageContent>> = {
  security: buildPages({
    en: ["Security // concrete controls", "Keep authority visible.", "Shimpz protects consequential work with explicit boundaries rather than an absolute security promise.", [
      ...
      ["Human authority at the boundary", "A reviewed Action can pause for one Team-owned human request. Approval, denial, expiry, and authentication remain owned by the platform boundary, not by Assistant code."],
