...
<Seo title={`${tr("login_welcome", lang)} · Shimpz`} description={tr("login_lead", lang)} {lang} />
...
<section class="wrap flex min-h-[calc(100dvh-15rem)] items-center justify-center py-12">
  <div class="w-full max-w-sm">
    <div class="mb-6 text-center">
      ...
      <h1 class="mono text-2xl font-extrabold tracking-tight">{tr("login_welcome", lang)}</h1>
      <p class="mt-2 text-sm leading-relaxed dim">{tr("login_lead", lang)}</p>
    </div>
    ...
    <div class="panel space-y-4">
      ...
      <label class="block">
        ...
        <div class="relative mt-1.5">
          {#if showPw}
            <input class="field !pr-16" type="text" autocomplete="current-password" bind:value={password} onkeydown={(e) => e.key === "Enter" && submit()} />
          {:else}
            <input class="field !pr-16" type="password" autocomplete="current-password" bind:value={password} onkeydown={(e) => e.key === "Enter" && submit()} />
          {/if}
    ...
    <p class="mt-5 text-center text-sm dim">
      <a class="underline decoration-dotted underline-offset-2 transition hover:text-[var(--color-fg)]" href="https://developers.shimpz.com/account/">
        {tr("creator_account_link", lang)}
      </a>
    </p>
  </div>
</section>
