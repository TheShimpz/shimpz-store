    ...
    assistants: [
      {
        ...
        powers: [{ id: "lookup", integrations: ["example"], human_requests: ["approval"] }],
      },
    ],
  };
}
...
test("fails closed on ambiguous or executable catalog data", () => {
  const mutations = [
    ...
    (value) => { delete value.assistants[0].powers[0].human_requests; },
    (value) => { value.assistants[0].powers[0].human_requests = ["input:magic"]; },
    (value) => { value.assistants[0].powers[0].human_requests = ["approval", "approval"]; },
    (value) => { value.assistants[0].powers = []; },
    (value) => { value.assistants[0].powers = Array.from({ length: 65 }, () => structuredClone(value.assistants[0].powers[0])); },
    (value) => { value.assistants[0].powers[0].integrations = Array.from({ length: 17 }, (_, index) => `integration-${index}`); },
    (value) => { value.assistants[0].powers[0].integrations = ["a".repeat(65)]; },
