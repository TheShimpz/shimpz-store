...
test("accepts Store context only from the exact parent at a named Admin origin", () => {
  ...
  const rejected = [
    ...
    { source: parentWindow, origin: "http://account-user@localhost:7777", data },
...
test("accepts only exact bounded installed-Assistant state from the loopback parent", () => {
  ...
  assert.deepEqual(
    ...
    { status: "ready", installed: ["shimpz-cloudflare", "salesnator"] },
  );
...
test("rejects untrusted, malformed, oversized, and state-bearing non-ready inventory", () => {
  ...
  const cases = [
    ...
    { ...exact, data: { ...exact.data, installed: ["shimpz-cloudflare", "shimpz-cloudflare"] } },
    ...
    { ...exact, data: null },
    { ...exact, data: [ASSISTANT_STORE_STATE_TYPE, 1, "ready", ["shimpz-cloudflare"]] },
  ];
...
test("requires authoritative ready state for install and uninstall", () => {
  ...
  assert.equal(assistantStoreActionForState("error", ["shimpz-cloudflare"], "shimpz-cloudflare"), "blocked");
  assert.equal(assistantStoreActionForState("unknown", ["shimpz-cloudflare"], "shimpz-cloudflare"), "blocked");
  assert.equal(assistantStoreActionForState("ready", ["shimpz-cloudflare"], "Hello-Pulse"), "blocked");
  assert.equal(assistantStoreActionForState("ready", "shimpz-cloudflare", "shimpz-cloudflare"), "blocked");
});

test("reconciles ACK and cancellation only against authoritative ready inventory", () => {
  assert.equal(
    shouldReconcileAssistantStoreAction("install", "pending", "ready", ["shimpz-cloudflare"], "shimpz-cloudflare"),
    true,
  );
  assert.equal(
    shouldReconcileAssistantStoreAction("uninstall", "pending", "ready", [], "shimpz-cloudflare"),
    true,
  );
  assert.equal(
    shouldReconcileAssistantStoreAction("install", "pending", "ready", [], "shimpz-cloudflare"),
    false,
  );
  assert.equal(
    shouldReconcileAssistantStoreAction("uninstall", "pending", "ready", ["shimpz-cloudflare"], "shimpz-cloudflare"),
    false,
  );
  assert.equal(
    shouldReconcileAssistantStoreAction("uninstall", "sent", "ready", ["shimpz-cloudflare"], "shimpz-cloudflare"),
    true,
  );
  assert.equal(
